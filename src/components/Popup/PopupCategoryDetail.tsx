...
const PopupCategoryDetail: React.FC<PopupCategoryDetailProps> = ({ cate, onPopupDetail, setOnPopupDetail }) => {
    return (
                        ...
                        <div className="sm:flex sm:items-start">
                            <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full pb-2">
                                ...
                                <div className="mt-4 border-t grid grid-cols-2 gap-4 p-8">
                                    ...
                                    <div>
                                        <span style={{ color: cate?.status === 1 ? 'green' : 'red', fontWeight: 'bold' }}>
                                            {cate?.status === 1 ? 'Active' : 'Inactive'}
                                        </span>
                                    </div>
                                    ...
                                    <div>
                                        <span>
                                            {typeof cate?.created === 'string'
                                                ? new Date(cate.created).toLocaleDateString('vi-VN')
                                                : cate?.created.toLocaleDateString('vi-VN')}
                                        </span>
                                    </div>
                                    <div>
                                        <span className="text-sm text-back-500 font-bold">
                                    ...
                                    <div>
                                        <span>{cate?.lastModifiedBy ?? 'Chưa có thay đổi'}</span>
                                    </div>
                                    <div>
                                        <span className="text-sm text-back-500 font-bold">
                                            Ngày chỉnh sửa
                                    ...
                                    <div>
                                        <span>
                                            {cate?.lastModified && cate.lastModified !== "" ? (
                                                typeof cate.lastModified === 'string'
                                                    ? new Date(cate.lastModified).toLocaleDateString('vi-VN')
                                                    : cate?.lastModified.toLocaleDateString('vi-VN')
                                            ) : "Chưa có thay đổi"}
                                        </span>
                                    </div>
                                    <div>
                                    ...
                                    <div className="w-auto flex gap-4">
                                        {cate?.status === 1 ? (
                                            <button className="text-xs w-auto border border-red-500 px-2 py-1 bg-orange-500 text-white-900 font-bold rounded-lg">Ngưng hoạt động</button>
                                        ) : (
                                            <button className="border border-green-500 p-1 bg-green-500 text-white-900 font-bold rounded-lg">Hoạt động</button>
                                        )}
                                        <button className="text-xs w-24 border border-red-500 p-1 bg-red-500 text-white-900 font-bold rounded-lg">Xoá</button>
