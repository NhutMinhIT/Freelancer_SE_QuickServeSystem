...
type FormUpdateIngredientNutritionsStepValues = {
    ingredientId: number;
    nutritionIds: {
        id: number,
    }[];
...
const PopupUpdateIngredientNutritions = ({
    ingredientId,
    isPopupOpen,
    closePopup,
}: PopupUpdateIngredientNutritionsProps) => {
    const dispatch = useAppDispatch();
    const [isLoading, setIsLoading] = useState(false);

    const { ingredientNutritionById } = useAppSelector(state => state.ingredientNutritions);
    ...
    const {
        ...
        reset,
        setValue,
    } = useForm<FormUpdateIngredientNutritionsStepValues>({
        resolver: yupResolver(schemaUpdateIngredientNutrition) as unknown as Resolver<FormUpdateIngredientNutritionsStepValues>,
    });
    ...
    useEffect(() => {
        if (ingredientNutritionById?.nutritions) {
            reset({
                ingredientId: ingredientId,
                nutritionIds: ingredientNutritionById.nutritions.map((nutrition) => ({ id: nutrition.id })),
            });
        }
    }, [ingredientNutritionById, reset, ingredientId]);
    
    useEffect(()=> {
        if(ingredientNutritionById?.nutritions && ingredientNutritionById?.nutritions.length !== 0){
            ingredientNutritionById?.nutritions.forEach((nutrition, index) => {
                setValue(`nutritionIds.${index}.id`, nutrition.id);
            })
        }
    }, [ingredientNutritionById?.nutritions, setValue, append])
    ...
    const onSubmit = (data: FormUpdateIngredientNutritionsStepValues) => {
        setIsLoading(true);
        ...
        isPopupOpen && (
            ...
            >
                <div className="flex items-center justify-between">
                    ...
                    >Cập nhật dinh dưỡng cho nguyên liệu</DialogTitle>
                    <XMarkIcon
                        height={24}
                        width={24}
                        ...
                        <div className="flex justify-between mt-4">
                            ...
                            >
                                {isLoading ? (
                                    ...
                                    "Cập nhật dinh dưỡng cho nguyên liệu"
                                )}
                            </button>
                        </div>
                    </form>
...
export default PopupUpdateIngredientNutritions;
